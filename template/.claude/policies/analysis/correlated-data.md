...
- **Continuous longitudinal / serial outcomes (the typical RCT): default to GLS**
  with a time-decaying correlation structure (AR(1) / continuous-time AR(1)) via
  `nlme::gls` / `rms::Gls` — it targets the group-level effect, uses standard ML
  with clean LR χ² tests, checks assumptions easily, and avoids the unrealistic
  exchangeability of a random intercept (Harrell, RMS ch. 7). Model **time as
  continuous** with >3 time points. Random effects are for subject-specific
  trajectories, not the average-treatment question.
